...
from apps.assignments.serializers import AssignmentSerializer, QuestionAttemptSerializer, AssignmentQuestionSerializer, AssignmentSubmissionSerializer

class AssignmentView(APIView):
    permission_classes = [IsAuthenticated]
# api view for create assignment by instructor and get assignment details for any student who enrolled in the course
    @extend_schema(
        tags=['Assignments'],
    ...
    def put(self, request, assignment_id):
        """Update an existing assignment (Instructor only)"""
        if not request.user.is_instructor:
            raise PermissionDenied("Only instructors can update assignments.")
...
class QuestionAttemptView(APIView):
    ...
    @extend_schema(
        tags=['Assignments'],
        request=QuestionAttemptSerializer,
        responses={
